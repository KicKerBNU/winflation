  ...
  <div class="flex min-h-screen items-center justify-center bg-gray-100 px-4 py-12 dark:bg-gray-950">
    <div class="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-8 shadow-sm dark:border-gray-800 dark:bg-gray-900">
      <div class="mb-6 text-center">
        ...
        <h1 class="mt-4 text-2xl font-bold text-gray-900 dark:text-white">{{ t('auth.login.title') }}</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ t('auth.login.subtitle') }}</p>
      </div>
      ...
      <p class="mt-6 text-center text-sm text-gray-500 dark:text-gray-400">
        ...
        <RouterLink to="/register" class="cursor-pointer font-semibold text-violet-600 hover:underline dark:text-violet-400">
          {{ t('auth.login.signUp') }}
        </RouterLink>
      </p>
    </div>
