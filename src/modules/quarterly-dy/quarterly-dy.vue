...
const filtered = computed<QuarterlyDyPick[]>(() => {
...
function exDivChip(p: QuarterlyDyPick): { text: string; tone: 'live' | 'soon' | 'idle' | 'none' } {
  const days = daysUntil(p.nextExDividendDate)
  if (days === null) return { text: t('quarterlyDy.nextExDividendUnknown'), tone: 'none' }
  if (days === 0) return { text: t('quarterlyDy.nextExDividendToday'), tone: 'live' }
  if (days < 0) {
    return p.lastDividendDate
      ? { text: t('quarterlyDy.lastDividendOn', { date: formatDate(p.lastDividendDate) }), tone: 'idle' }
      : { text: t('quarterlyDy.nextExDividendUnknown'), tone: 'none' }
  }
  if (days <= 30) return { text: t('quarterlyDy.nextExDividendIn', { days }), tone: 'soon' }
  return { text: t('quarterlyDy.nextExDividendOn', { date: formatDate(p.nextExDividendDate) }), tone: 'idle' }
}

function routeFor(ticker: string) {
  return { name: 'quarterly-dy-detail', params: { ticker: encodeURIComponent(ticker) } }
}
</script>
...
<template>
  <div class="p-4 pt-16 sm:px-6 sm:pb-6 lg:p-8">
    <div class="mx-auto max-w-6xl">
      ...
      <!-- Header -->
      <header class="mb-8">
        <div class="inline-flex items-center gap-2 rounded-full bg-indigo-100 px-3 py-1 text-[11px] font-semibold uppercase tracking-widest text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300">
          <FontAwesomeIcon icon="rotate" class="text-[10px]" />
          <span>{{ t('quarterlyDy.refreshesWeekly') }}</span>
          <template v-if="formattedDate">
            · <span>{{ t('quarterlyDy.updatedAt', { date: formattedDate }) }}</span>
          </template>
        </div>
      ...
      <div class="mb-6 flex flex-col gap-3 lg:flex-row lg:items-end lg:gap-4">
        ...
        <label class="block flex-1">
          <span class="sr-only">{{ t('quarterlyDy.searchPlaceholder') }}</span>
          <div class="relative">
            <FontAwesomeIcon
            ...
            <input
              v-model="search"
              type="search"
              :placeholder="t('quarterlyDy.searchPlaceholder')"
              class="block w-full rounded-lg border border-gray-300 bg-white py-2 pl-9 pr-3 text-sm text-gray-900 transition-colors focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
            />
          </div>
        </label>
        ...
        <label class="block lg:w-72">
          <span class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
            {{ minYield > 0 ? t('quarterlyDy.yieldFilterLabel') + ` ≥ ${minYield}%` : t('quarterlyDy.yieldFilterAll') }}
          </span>
          <input
            ...
            max="12"
            step="0.5"
            class="block w-full cursor-pointer accent-indigo-600"
          />
        </label>
        ...
        <label class="block lg:w-56">
          <span class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
            {{ t('quarterlyDy.sortLabel') }}
          </span>
          <select
            v-model="sortMode"
            class="block w-full cursor-pointer rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 transition-colors focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
          >
            <option value="yield-desc">{{ t('quarterlyDy.sortYieldDesc') }}</option>
            <option value="yield-asc">{{ t('quarterlyDy.sortYieldAsc') }}</option>
            <option value="market-cap">{{ t('quarterlyDy.sortMarketCap') }}</option>
            <option value="risk">{{ t('quarterlyDy.sortRisk') }}</option>
          </select>
        </label>
      </div>
      ...
      >
        <FontAwesomeIcon icon="rotate" class="mb-3 text-2xl text-gray-300" />
        <p class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ t('quarterlyDy.empty') }}</p>
      </div>

      <template v-else>
        <p class="mb-3 text-xs uppercase tracking-widest text-gray-400">
          {{ t('quarterlyDy.pickCount', { count: filtered.length }) }}
        </p>
        ...
        <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          <RouterLink
            ...
            class="group flex cursor-pointer flex-col rounded-2xl border border-gray-200 bg-white p-5 transition-all hover:-translate-y-0.5 hover:border-indigo-300 hover:shadow-lg dark:border-gray-800 dark:bg-gray-900 dark:hover:border-indigo-700"
          >
            <!-- Top: risk badge + ex-div chip -->
            <div class="mb-3 flex items-start justify-between gap-2">
              ...
              <span
                ...
                :class="{
                  'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300': exDivChip(p).tone === 'live',
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="min-w-0 flex-1">
                ...
                <div class="mt-0.5 flex flex-wrap items-center gap-1.5 text-[11px] text-gray-500 dark:text-gray-400">
                  <span class="font-mono font-semibold text-indigo-600 dark:text-indigo-400">{{ p.ticker }}</span>
            ...
            <div class="mt-4 flex items-end justify-between border-t border-gray-100 pt-3 dark:border-gray-800">
              <div>
                <div class="text-[10px] uppercase tracking-wider text-gray-400">{{ t('quarterlyDy.yieldLabel') }}</div>
                <div class="text-3xl font-bold tabular-nums text-indigo-600 dark:text-indigo-400">
                  {{ p.dividendYield.toFixed(2) }}<span class="text-base text-gray-400">%</span>
                </div>
                <div class="text-[11px] text-gray-500 dark:text-gray-400">
                  {{ formatPrice(p.annualDividend, p.currency) }} / yr · {{ t('quarterlyDy.paymentsPerYear', { n: p.paymentFrequency }) }}
                </div>
              </div>
              <div class="text-right">
                <div class="text-[10px] uppercase tracking-wider text-gray-400">{{ t('quarterlyDy.marketCap') }}</div>
