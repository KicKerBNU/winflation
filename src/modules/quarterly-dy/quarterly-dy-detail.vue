...
import { useQuarterlyDyStore } from './store/quarterly-dy.store'
...
import type { QuarterlyDyPick, RiskTier, AssetClass } from './domain/quarterly-dy.types'
...
const store = useQuarterlyDyStore()
...
const pick = computed<QuarterlyDyPick | null>(
...
const riskConfig: Record<RiskTier, { label: string; desc: string; pill: string; iconColor: string; icon: string }> = {
  low: {
    label: 'quarterlyDy.riskLow',
    desc: 'quarterlyDy.riskLowDesc',
  ...
  medium: {
    label: 'quarterlyDy.riskMedium',
    desc: 'quarterlyDy.riskMediumDesc',
  ...
  high: {
    label: 'quarterlyDy.riskHigh',
    desc: 'quarterlyDy.riskHighDesc',
    pill: 'border-red-200 bg-red-50 text-red-700 dark:border-red-800/40 dark:bg-red-900/20',
...
const assetClassLabel: Record<AssetClass, string> = {
  'dividend-aristocrat': 'quarterlyDy.assetAristocrat',
  'dividend-achiever': 'quarterlyDy.assetAchiever',
  'utility': 'quarterlyDy.assetUtility',
  'bank': 'quarterlyDy.assetBank',
  'energy': 'quarterlyDy.assetEnergy',
  'reit': 'quarterlyDy.assetReit',
}
...
const beatPalette: Record<BeatVerdict, { card: string; iconColor: string; icon: string; valueColor: string }> = {
  beats: {
    card: 'border-indigo-200 bg-indigo-50 dark:border-indigo-800/40 dark:bg-indigo-900/20',
...
// ── Distributions (grouped by year, newest first) ─────────────────────────────
...
function buildPriceChart() {
  ...
  const fillColor = isDark ? 'rgba(99,102,241,0.18)' : 'rgba(99,102,241,0.12)'
  ...
  priceChart = new Chart(priceCanvas.value, {
    ...
    data: {
      datasets: [
        {
          label: t('quarterlyDy.priceLabel'),
          data: linePoints,
          parsing: false,
        ...
        {
          label: t('quarterlyDy.dividendMarkerLabel'),
          ...
          callbacks: {
            ...
            label: (ctx) => {
              ...
              if (ctx.datasetIndex === 1 && raw._amount !== undefined) {
                return [
                  ` ${t('quarterlyDy.dividendTooltipAmount', { amount: formatPrice(raw._amount, currency) })}`,
                  ` ${t('quarterlyDy.priceLabel')}: ${formatPrice(raw.y, currency)}`,
                ]
              }
              return ` ${t('quarterlyDy.priceLabel')}: ${formatPrice(raw.y, currency)}`
            },
          },
        },
      },
...
<template>
  <div class="p-4 pt-16 sm:px-6 sm:pb-6 lg:p-8">
    <button
      class="mb-6 flex cursor-pointer items-center gap-2 text-sm text-gray-500 transition-colors hover:text-indigo-600 dark:text-gray-400 dark:hover:text-indigo-400"
      @click="router.push('/quarterly-dy')"
    >
      <FontAwesomeIcon icon="arrow-left" />
      {{ t('quarterlyDy.backToList') }}
    </button>
    ...
    >
      ...
      <p class="text-sm text-gray-600 dark:text-gray-400">{{ t('quarterlyDy.notFound') }}</p>
    </div>
    ...
    <template v-else-if="pick">
      ...
      <div class="mb-6 flex flex-wrap items-start gap-4">
        ...
        <div class="min-w-0 flex-1">
          <div class="mb-1 flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-widest text-indigo-600 dark:text-indigo-400">
            <span
            ...
            >
              ...
              {{ t('quarterlyDy.riskTitle') }}: {{ t(riskConfig[pick.riskTier].label) }}
            </span>
          ...
          <div class="mt-1 flex flex-wrap items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
            <span class="font-mono font-semibold text-indigo-600 dark:text-indigo-400">{{ pick.ticker }}</span>
      ...
      >
        <div class="mb-3 flex items-baseline justify-between gap-3">
          <h2 class="text-sm font-semibold text-gray-700 dark:text-gray-300">
            {{ t('quarterlyDy.priceHistoryTitle') }}
          </h2>
          <span v-if="priceHistorySinceLabel" class="text-[11px] text-gray-400">
            {{ t('quarterlyDy.priceHistorySince', { date: priceHistorySinceLabel }) }}
          </span>
        </div>
      ...
      <div class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div class="rounded-2xl border border-indigo-200 bg-indigo-50 p-4 dark:border-indigo-800/40 dark:bg-indigo-900/20">
          <p class="text-xs text-indigo-600 dark:text-indigo-400">{{ t('quarterlyDy.yieldLabel') }}</p>
          <p class="mt-1 text-3xl font-bold text-indigo-700 dark:text-indigo-300">
            {{ pick.dividendYield.toFixed(2) }}%
          </p>
          <p class="mt-0.5 text-xs text-indigo-600/80 dark:text-indigo-400/80">
            {{ t('quarterlyDy.paymentsPerYear', { n: pick.paymentFrequency }) }}
          </p>
        </div>
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
          <p class="text-xs text-gray-400">{{ t('quarterlyDy.annualDividend') }}</p>
          <p class="mt-1 text-xl font-bold text-gray-900 dark:text-white">
            {{ formatPrice(pick.annualDividend, pick.currency) }}
          </p>
          <p class="mt-0.5 text-xs text-gray-400">
            {{ t('quarterlyDy.perPaymentLabel') }}: {{ formatPrice(pick.annualDividend / Math.max(pick.paymentFrequency, 1), pick.currency) }}
          </p>
        </div>
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
          <p class="text-xs text-gray-400">{{ t('quarterlyDy.price') }}</p>
          <p class="mt-1 text-xl font-bold text-gray-900 dark:text-white">
            {{ formatPrice(pick.currentPrice, pick.currency) }}
          </p>
          <p
            class="mt-0.5 flex items-center gap-1 text-xs"
            :class="pick.priceChangePercent >= 0 ? 'text-indigo-600 dark:text-indigo-400' : 'text-red-500 dark:text-red-400'"
          >
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
          <p class="text-xs text-gray-400">{{ t('quarterlyDy.marketCap') }}</p>
          <p class="mt-1 text-xl font-bold text-gray-900 dark:text-white">
      ...
      <!-- Calendar: next ex-div + next pay + last paid -->
      <div class="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
          <p class="text-[10px] uppercase tracking-wider text-gray-400">{{ t('quarterlyDy.nextExDividendLabel') }}</p>
          ...
          <p
            ...
            :class="daysUntil(pick.nextExDividendDate)! <= 30 && daysUntil(pick.nextExDividendDate)! >= 0 ? 'font-semibold text-amber-600 dark:text-amber-400' : 'text-gray-400'"
          >
            <template v-if="daysUntil(pick.nextExDividendDate)! === 0">{{ t('quarterlyDy.nextExDividendToday') }}</template>
            <template v-else-if="daysUntil(pick.nextExDividendDate)! > 0">
              {{ t('quarterlyDy.nextExDividendIn', { days: daysUntil(pick.nextExDividendDate) }) }}
            </template>
          </p>
        </div>
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
          <p class="text-[10px] uppercase tracking-wider text-gray-400">{{ t('quarterlyDy.nextPaymentLabel') }}</p>
          <p class="mt-1 text-base font-semibold text-gray-900 dark:text-white">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
          <p class="text-[10px] uppercase tracking-wider text-gray-400">{{ t('quarterlyDy.lastDividendLabel') }}</p>
          <p class="mt-1 text-base font-semibold text-gray-900 dark:text-white">
          ...
          <section v-for="g in distributionsByYear" :key="g.year">
            <header class="mb-2 flex items-baseline justify-between gap-3 border-b border-gray-100 pb-1 dark:border-gray-800">
              ...
              <span class="text-[11px] text-gray-400">
                {{ t('quarterlyDy.distributionsYearTotal', { total: formatPrice(g.total, pick.currency), count: g.rows.length }) }}
              </span>
            </header>
            <ul class="divide-y divide-gray-100 dark:divide-gray-800">
              ...
              >
                ...
                <div class="h-2 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
                  <div class="h-full rounded-full bg-indigo-500/70 dark:bg-indigo-400/70" :style="{ width: barWidthPct(d.amount) }" />
                </div>
      ...
      >
        ...
        <div class="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
          ...
          <div v-if="wht.treaty !== null" class="flex items-baseline gap-1.5">
            ...
            <span class="font-mono text-base font-bold text-indigo-700 dark:text-indigo-400">{{ fmtPct(wht.treaty) }}</span>
          </div>
          <span
