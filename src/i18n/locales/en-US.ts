export default {
  nav: {
    dashboard: 'Dashboard',
    interestRate: 'Interest Rate',
    inflation: 'Inflation',
    dividends: 'Dividends',
    aiRecommendation: 'Picks',
    monthlyDy: 'Monthly DY',
    quarterlyDy: 'Quarterly DY',
    followed: 'Followed',
    minerals: 'Minerals',
  },
  dashboard: {
    title: 'Overview',
    subtitle: 'Your inflation-beating financial dashboard',
    ecbRate: 'ECB Deposit Rate',
    depositFacility: 'Deposit Facility',
    countriesTracked: 'Countries Tracked',
    europeanCountries: 'European countries',
    stocksAboveRate: 'Stocks Above ECB Rate',
    beatingEcbRate: 'Beating the ECB rate',
    beatingInflation: 'Beating Inflation',
    beatingLocalInflation: 'Beating local inflation',
    topOpportunities: 'Top Dividend Opportunities',
    upcomingExDividends: 'Upcoming Ex-Dividends',
    upcomingExDividendsSubtitle: 'Companies from AI Picks and Monthly DY with announced ex-dividend dates and upcoming payments.',
    next7Days: 'Next 7 days',
    next30Days: 'Next 30 days',
    exDividendDate: 'Ex-dividend',
    paymentValue: 'Payment value',
    dividendYield: 'DY',
    paymentsPerYear: 'Payments',
    paymentFrequency: '{count}x / year',
    paymentTomorrow: 'Pays tomorrow',
    paymentInDays: 'Pays in {days} days',
    paymentDate: 'Pays {date}',
    sourceMonthlyDy: 'Monthly DY',
    sourceQuarterlyDy: 'Quarterly DY',
    sourceAiPicks: 'AI Picks',
    viewMonthlyDy: 'View Monthly DY',
    noUpcomingExDividends: 'No announced ex-dividend payments in the next 30 days.',
    highestInflation: 'Highest Inflation Countries',
    lowestInflation: 'Lowest Inflation Countries',
    viewAll: 'View all',
    trendRising: 'Rising',
    trendFalling: 'Falling',
    trendStable: 'Stable',
  },
  interestRate: {
    title: 'ECB Interest Rates',
    subtitle: 'European Central Bank key interest rates',
    deposit: 'Deposit Facility',
    main: 'Main Refinancing',
    marginal: 'Marginal Lending',
    rateType: 'Rate Type',
    value: 'Value',
    effectiveDate: 'Effective Date',
    loading: 'Loading interest rates...',
    whatAreRates: 'What are ECB interest rates?',
    ratesDescription:
      'The ECB sets three key interest rates. The deposit facility rate is the rate banks receive for depositing money with the ECB overnight. The main refinancing rate is the cost for banks to borrow from the ECB for one week. The marginal lending rate is the cost for overnight borrowing from the ECB. These rates directly influence inflation and borrowing costs across the Eurozone.',
  },
  inflation: {
    title: 'Inflation by Country',
    subtitle: 'Annual CPI inflation rate across European countries',
    country: 'Country',
    rate: 'Rate',
    trend: 'Trend',
    vsEcb: 'vs ECB Rate',
    date: 'Date',
    searchPlaceholder: 'Search country...',
    filterAll: 'All',
    filterHigh: 'High (>4%)',
    filterMedium: 'Medium (2–4%)',
    filterLow: 'Low (<2%)',
    loading: 'Loading inflation data...',
    dataLag: 'Showing data for {date} — Eurostat API publishes with a ~3 month delay.',
    noResults: 'No countries match your search.',
  },
  inflationCountry: {
    back: 'Back to all countries',
    subtitle: 'Monthly HICP inflation — annual rate of change',
    loading: 'Loading historical data...',
    error: 'Failed to load inflation history.',
    latest: 'Latest',
    average: '10-year avg.',
    tenYears: 'Last 10 years',
    peak: 'Peak',
    low: 'Low',
    chartTitle: 'Inflation rate over the last 10 years (%)',
    debtLabel: 'Government Debt',
    debtOfGdp: 'of GDP',
    debtSince: 'since {year}',
    aiInsightsTitle: 'AI Economic Briefing',
    prioritiesTitle: 'Top Priorities',
    problemsTitle: 'Main Problems',
    opportunitiesTitle: 'Opportunities',
  },
  dividendCompany: {
    back: 'Back to dividends',
    price: 'Price',
    yieldLabel: 'Dividend Yield',
    annualDividend: 'Annual Dividend',
    marketCap: 'Market Cap',
    beta: 'Beta',
    betaLow: 'below market',
    betaHigh: 'above market',
    weekHigh: '52-week High',
    weekLow: '52-week Low',
    chartTitle: 'Yield vs Benchmarks',
    ecbRateLabel: 'ECB Rate',
    inflationLabel: 'Local Inflation',
    about: 'About',
    ceoLabel: 'CEO',
    websiteLabel: 'Visit website',
    proTitle: 'Why consider it',
    conTitle: 'Risk to watch',
    loading: 'Loading company data...',
    error: 'Failed to load company data.',
    pro: {
      beatsInflation: 'Dividend yield of {yield}% exceeds local inflation ({inflation}%), protecting your purchasing power in real terms.',
      beatsEcb: 'Yield of {yield}% outperforms the ECB deposit rate of {rate}%, making it more attractive than risk-free savings.',
      stableSector: '{sector} companies are historically among the most reliable dividend payers, with stable cash flows across economic cycles.',
      lowBeta: 'Low beta of {beta} indicates below-average price volatility — less turbulent than the broader market.',
      established: 'A large-cap company with market capitalisation of {cap} and an established track record of returning capital to shareholders.',
    },
    con: {
      highYield: 'A yield above 8% can indicate elevated risk or an anticipated dividend cut — always verify payout ratio and earnings coverage.',
      cyclical: '{sector} is a cyclical sector — dividends tend to fluctuate and may be reduced during economic downturns.',
      highBeta: 'Beta of {beta} indicates above-average price volatility — the stock can swing significantly with broader market moves.',
      belowEcb: 'Current yield sits below the ECB deposit rate of {rate}% — the dividend alone does not yet outperform risk-free savings.',
      concentration: 'Single-stock exposure concentrates risk. Consider whether this holding is diversified across sectors and geographies in your portfolio.',
    },
  },
  dividends: {
    title: 'Dividend Tracker',
    subtitle: 'European stocks with high dividend yield — find what beats inflation',
    company: 'Company',
    country: 'Country',
    yield: 'Yield',
    sector: 'Sector',
    exchange: 'Exchange',
    status: 'Status',
    ecbRateLabel: 'ECB Rate',
    legendBeatsInflation: 'Beats local inflation',
    legendBeatsEcb: 'Beats ECB rate',
    beatsEcb: 'Beats ECB Rate',
    beatsInflation: 'Beats Inflation',
    searchPlaceholder: 'Search company or ticker...',
    filterAll: 'All',
    filterBeatsRate: 'Beats ECB Rate',
    filterBeatsInflation: 'Beats Inflation',
    loading: 'Loading dividend data...',
    noResults: 'No stocks match your search.',
  },
  aiRecommendation: {
    title: 'AI Dividend Picks',
    subtitle: 'Top 10 European high-yield stocks — AI-curated, refreshed daily',
    updatedAt: 'Updated {date}',
    refreshesDaily: 'Refreshes daily at 8:00 UTC',
    loading: 'Loading AI recommendations...',
    empty: 'No recommendations yet. The AI generates fresh picks once a day — check back soon.',
    error: 'Failed to load AI recommendations.',
    proTitle: 'Why invest',
    conTitle: 'Key risk',
    statusBullish: 'Bullish',
    statusNeutral: 'Neutral',
    statusBearish: 'Bearish',
    yieldHistory: 'Yield History (5 yr)',
    dividendsPerYear: 'Dividends per Year',
    yieldLabel: 'Dividend Yield',
    price: 'Price',
    annualDiv: 'Annual Dividend',
    marketCap: 'Market Cap',
    kicker: 'AI weekly · {date}',
    kickerNoDate: 'AI weekly',
    headlinePre: 'High-yield Europe,',
    headlineAccent: 'curated',
    editorialSubtitle: "Ten European dividend payers, ranked by yield. Here's the AI's thesis, not the spreadsheet.",
    pickOfTheWeek: 'Pick of the week',
    rankNumber: 'Rank #{rank}',
    alsoRanking: 'Also in the ranking',
    morePicks: '{count} more picks',
    readFullAnalysis: 'Read full analysis',
    backToPicks: 'Back to AI Picks',
    companyNotFound: 'Company not found in the current picks.',
    loadingHistory: 'Loading history...',
    evolutionTitle: 'Yield & dividend evolution',
    yearlyYieldLabel: '{yield}% yield',
    perShareYear: 'per share / year',
    paymentDates: 'Payment dates',
    paidOn: 'Paid {date}',
    realYieldLabel: 'Real yield',
    realYieldHint: '{nominal}% yield − {inflation}% HICP ({country})',
    realYieldHintNeg: 'Inflation outpaces this yield in {country}',
    realYieldNoData: 'HICP not tracked for {country} — showing nominal yield',
    inflationLabel: 'Inflation',
    nominalYieldLabel: 'Nominal yield',
    hicpRate: 'HICP {rate}%',
    hicpAsOf: 'HICP as of {date}',
    inflationBeatTitle: 'vs. your inflation',
    inflationBeatBeats: 'Beats your inflation by {amount}%',
    inflationBeatMatches: 'Roughly matches your inflation',
    inflationBeatLoses: 'Loses to your inflation by {amount}%',
    inflationBeatShort: 'Beats inflation +{amount}%',
    inflationBeatShortNeg: 'Loses to inflation −{amount}%',
    inflationBeatShortMatch: 'Matches inflation',
    inflationBeatHint: 'Nominal yield {nominal}% − HICP {inflation}% in {country}',
    inflationCtaTitle: 'See how each pick beats your inflation',
    inflationCtaBody: 'Set your tax country in Settings and we’ll show, per pick, whether you’re winning or losing against your local inflation.',
    inflationCtaLink: 'Open settings',
    tierLegendTitle: 'Strategy tier',
    tierLegendBody: 'Each pick passed one of three quality screens. Conservative is strictest; Permissive is most lenient.',
    tierConservative: 'Conservative',
    tierModerate: 'Moderate',
    tierPermissive: 'Permissive',
    tierConservativeShort: 'Conservative strategy',
    tierModerateShort: 'Moderate strategy',
    tierPermissiveShort: 'Permissive strategy',
    tierConservativeDesc: 'Passes our strictest screen — strong sustainability, growth, and profitability across the board.',
    tierModerateDesc: 'Passes our standard screen — solid fundamentals with room for some variability.',
    tierPermissiveDesc: 'Passes our broadest screen — high yield with weaker or incomplete metrics; verify carefully.',
    disclaimer: {
      complianceLead: 'Informational only · not financial advice',
      complianceBody: 'AI-generated content. Figures may be inaccurate or outdated. Verify with primary sources and consult a licensed advisor before any investment decision.',
      aboutTitle: 'About these picks',
      aboutMethodologyTitle: 'Methodology',
      aboutMethodology: 'The list is generated daily by Google Gemini, an AI language model, from a financial-analyst-style prompt that targets European large-cap dividend payers above 2× the current ECB deposit rate. Numeric figures (price, yield, market cap, dividend history) are AI-generated and may be inaccurate or outdated — verify against primary sources (the issuer\'s investor-relations site, your broker, or official exchange data) before acting.',
      aboutNotAdviceTitle: 'Not financial advice',
      aboutNotAdvice: 'Nothing here is a personal investment recommendation or advice. Past performance does not predict future results. Consult a licensed financial advisor before any investment decision.',
      aboutConflictsTitle: 'Conflicts of interest',
      aboutConflicts: 'The publisher of winflation.eu may hold positions in some of the listed securities.',
    },
  },
  footer: {
    disclaimer: 'winflation.eu is an informational and educational tool. Nothing on this site is financial, investment, or tax advice. Data may be inaccurate or outdated — verify with primary sources before any investment decision.',
  },
  auth: {
    or: 'or',
    email: 'Email',
    password: 'Password',
    name: 'Full name',
    menu: {
      login: 'Log in',
      logout: 'Log out',
      settings: 'Settings',
    },
    login: {
      title: 'Welcome back',
      subtitle: 'Sign in to track the companies you follow.',
      google: 'Continue with Google',
      submit: 'Sign in',
      noAccount: "Don't have an account?",
      signUp: 'Sign up',
    },
    register: {
      title: 'Create your account',
      subtitle: 'Start following the European dividend stocks you care about.',
      google: 'Sign up with Google',
      submit: 'Create account',
      passwordHint: 'At least 6 characters.',
      haveAccount: 'Already have an account?',
      signIn: 'Sign in',
    },
    errors: {
      invalidEmail: 'That email address is not valid.',
      userDisabled: 'This account has been disabled.',
      invalidCredentials: 'Incorrect email or password.',
      emailInUse: 'An account with this email already exists.',
      weakPassword: 'Password must be at least 6 characters.',
      popupClosed: 'Sign-in window closed before completing.',
      network: 'Network error — please check your connection.',
      tooManyRequests: 'Too many attempts. Try again in a few minutes.',
      generic: 'Something went wrong. Please try again.',
    },
  },
  minerals: {
    title: 'Rare Minerals',
    subtitle: 'Where the world sources the metals that power EVs, wind turbines, and AI hardware',
    loading: 'Loading minerals data...',
    dataAsOf: 'Data as of {date}',
    warning: {
      title: 'Key warning for investors — price takers',
      body: "These companies are price takers. Their stock prices move violently with the global price of NdPr (neodymium-praseodymium). If China floods the market to crush prices, margins shrink instantly. As of 2026, Western governments are providing subsidies and floor prices to keep non-Chinese supply viable.",
    },
    mapTitle: 'Global reserves & operations',
    mapHint: 'Click a marker to see minerals and companies active in that country.',
    countriesTitle: 'Key countries',
    countriesHint: 'Ranked by number of critical minerals produced.',
    mineralsTitle: 'Critical minerals for 2026',
    usedFor: 'Used for',
    viewCountry: 'View country details',
    companiesCount: '{count} companies | {count} company | {count} companies',
    why: {
      title: 'Why rare minerals matter to investors',
      body: 'China still controls ~60% of global rare-earth production and an even tighter grip on refined oxides and magnet making. As EVs, wind turbines, robotics and defense scale up, Western governments are backing new mines and refineries outside China — creating a rare investable shift in a market that was structurally monopolised for two decades.',
    },
    category: {
      all: 'All',
      magnet: 'Magnet metals',
      battery: 'Battery',
      other: 'Other',
    },
    entries: {
      Nd: {
        name: 'Neodymium',
        description: 'Part of the "power pair" with Praseodymium, forming the backbone of permanent magnets in electric motors.',
        use: 'Permanent magnets for EV drivetrains, wind turbines, and consumer electronics.',
      },
      Pr: {
        name: 'Praseodymium',
        description: 'Paired with Neodymium in NdPr alloys — together they dominate the high-performance magnet market.',
        use: 'High-strength permanent magnets, aerospace alloys.',
      },
      Dy: {
        name: 'Dysprosium',
        description: 'A "heavy" rare earth added to magnets so they keep their strength at high temperatures.',
        use: 'High-temperature magnets for EVs and defense systems.',
      },
      Tb: {
        name: 'Terbium',
        description: 'Another heavy rare earth that improves magnet coercivity — critical for high-performance motors.',
        use: 'Heat-resistant magnets, phosphors in displays.',
      },
      Li: {
        name: 'Lithium',
        description: 'The lightest metal and the cornerstone of modern battery chemistry — not technically a rare earth but grouped as a "rare mineral".',
        use: 'Lithium-ion batteries for EVs and grid storage.',
      },
      Co: {
        name: 'Cobalt',
        description: 'Stabilises cathodes in lithium-ion batteries, extending life and safety.',
        use: 'Battery cathodes (NMC / NCA chemistries), superalloys.',
      },
      Graphite: {
        name: 'Graphite',
        description: 'Essential for battery anodes. China dominates the "spherical" graphite refining stage globally.',
        use: 'Anodes in virtually all lithium-ion batteries.',
      },
      La: {
        name: 'Lanthanum',
        description: 'A light rare earth used in catalysts and specialty optical glass.',
        use: 'Refinery catalysts, camera lenses, NiMH batteries.',
      },
      Ce: {
        name: 'Cerium',
        description: 'The most abundant rare earth, often co-produced with Lanthanum from monazite ores.',
        use: 'Glass polishing, auto catalysts, alloys.',
      },
      Monazite: {
        name: 'Monazite',
        description: 'A rare-earth-bearing mineral sand, also containing thorium. India holds huge reserves in its beach sands.',
        use: 'Feedstock for mixed rare-earth concentrates.',
      },
    },
    countries: {
      CN: {
        name: 'China',
        status: 'Dominant producer of all 17 REEs and graphite; controls global pricing and refining.',
        companies: {
          chinaRareEarthGroup: 'State-owned mega-firm created by merging several smaller companies to control the domestic market.',
          shengheResources: 'Major global player that also invests in overseas projects (US, Greenland).',
        },
      },
      VN: {
        name: 'Vietnam',
        status: "World's second-largest reserves (~22M tonnes); production scaling rapidly with Western backing.",
        companies: {
          vtre: 'Working with Australian and Korean firms to restart the Dong Pao mine, one of the largest in the world.',
          blackstone: 'Actively exploring nickel and associated rare earth projects in the Ta Khoa district.',
        },
      },
      BR: {
        name: 'Brazil',
        status: 'Massive deposits in Minas Gerais; recently became a "Tier 1" supplier to the US.',
        companies: {
          serraVerde: 'Operates the Pela Ema mine, one of the only sources outside Asia for the four magnet metals (Nd, Pr, Tb, Dy).',
          appia: 'Actively exploring the high-grade Cachoeira de Paita project.',
        },
      },
      AU: {
        name: 'Australia',
        status: 'Primary non-Chinese source of light rare earths.',
        companies: {
          lynas: {
            description: "World's largest rare-earth producer outside China — operates Mount Weld mine and a separation plant in Malaysia.",
            catalyst: 'Expansion of the Malaysian processing plant; scaling NdPr output to meet long-term allied demand.',
            contracts: 'Strategic supply contracts with the U.S. Department of Defense and Japanese industrial partners.',
          },
          iluka: "Building Australia's first fully integrated rare-earth refinery.",
        },
      },
      US: {
        name: 'United States',
        status: 'Scaling domestic magnet production to match its mining output.',
        companies: {
          mpMaterials: {
            description: 'Operates the Mountain Pass mine in California; moving into "mine-to-magnet" production to bypass Chinese factories.',
            catalyst: 'Ramp-up of the $200M "10X" magnetics facility in Texas — fully vertically integrated production from ore to finished magnets.',
            contracts: 'Major offtake agreement with General Motors to supply permanent magnets for EV motors.',
          },
          usaRareEarth: {
            description: 'Developing the Round Top project in Texas; in April 2026 acquired the Pela Ema mine (Brazil) for $2.8B — a "Mine-to-Magnet" strategy feeding U.S. processing facilities.',
            catalyst: 'Integration of the Brazilian Pela Ema acquisition; production expected to hit 6,400 tonnes of rare earth oxides annually by 2027.',
            contracts: 'Vertically integrated supply: Brazilian ore feeds U.S. processing and magnet-making facilities — bypasses Chinese refining.',
          },
        },
      },
      IN: {
        name: 'India',
        status: 'Massive beach sand deposits (monazite); currently focused on domestic high-tech use.',
        companies: {},
      },
      CA: {
        name: 'Canada',
        status: 'Expanding rapidly in the Northwest Territories and Quebec.',
        companies: {
          vitalMetals: "Canada's first rare earth producer, focused on the Nechalacho project.",
          neo: 'Canadian-HQ company operating processing plants globally (including Estonia) to produce high-value powders and magnets.',
        },
      },
      ZA: {
        name: 'South Africa',
        status: 'Home to Steenkampskraal — the highest-grade rare earth deposit in the world (~14.5% REO vs. the 1-2% typical elsewhere).',
        companies: {
          smm: 'Operator of the Steenkampskraal mine, rolling out a six-phase plan to mine, process and separate the metals domestically.',
        },
      },
      MW: {
        name: 'Malawi',
        status: "Fast-rising magnet-metals player; the Songwe Hill project is among the few globally to reach definitive feasibility.",
        companies: {
          mkango: {
            description: 'Building a "mine-to-market" pipeline from Songwe Hill — mining in Malawi, separation in Poland, selling directly to European carmakers.',
            catalyst: 'Definitive Feasibility Study completed March 2026. Construction slated for 2027, first production in 2029. Financing close is the near-term catalyst.',
            contracts: 'Vertical integration: Songwe Hill concentrate ships to a dedicated separation plant in Poland for European OEM offtake.',
          },
        },
      },
      NA: {
        name: 'Namibia',
        status: 'Massive reserves and a rare source of the heavy rare earths (Dy, Tb) needed for high-temperature magnets.',
        companies: {
          bannerman: 'Primarily a uranium developer (Etango Project), but supports key infrastructure for the broader rare-minerals sector.',
          ncm: 'Developing the Lofdal project, specifically rich in heavy rare earths for high-temp magnets.',
        },
      },
      BI: {
        name: 'Burundi',
        status: 'Home to Gakara — historically the highest-purity rare earth concentrate in the world.',
        companies: {
          rainbow: {
            description: 'Operator of the Gakara project; top-tier asset despite political friction with the government over profit-sharing. Also advancing a low-cost phosphogypsum extraction project in South Africa.',
            catalyst: 'Successful pilot of waste-pile (phosphogypsum) extraction tech — far lower capex than deep-rock mining.',
            contracts: '$50M investment secured from the U.S. International Development Finance Corporation (DFC) — strong Western political backing.',
          },
        },
      },
      CD: {
        name: 'DR Congo',
        status: "Produces over 70% of the world's cobalt — the critical stabiliser in lithium-ion battery cathodes.",
        companies: {
          glencore: 'Major operator of large-scale copper-cobalt mines in Katanga — one of the largest cobalt producers globally.',
          cmoc: 'Chinese-backed operator of Tenke Fungurume and Kisanfu — among the largest cobalt mines in the world.',
        },
      },
      ML: {
        name: 'Mali',
        status: 'Became a major lithium producer in late 2025 / early 2026 as the Goulamina project reached production.',
        companies: {
          ganfeng: 'Operator of the Goulamina lithium project — one of the largest spodumene deposits in Africa.',
          leoLithium: 'Development partner on Goulamina, a flagship lithium project supplying the EV battery market.',
        },
      },
    },
  },
  mineralsCountry: {
    back: 'Back to minerals',
    notFound: 'Country not found in the minerals dataset.',
    mineralsTitle: 'Minerals found here',
    companiesTitle: 'Companies operating on the ground',
    companiesEmpty: 'No major public operators tracked yet in this country.',
    mineralsCount: '{count} minerals | {count} mineral | {count} minerals',
    companiesCount: '{count} companies | {count} company | {count} companies',
    financials: {
      profitability: 'Profitability',
      dividend: 'Dividend',
      mineLife: 'Mine life',
      risk: 'Risk',
      revenueYoY: 'Revenue YoY',
      netIncome: 'Net income',
      primaryCatalyst: 'Primary catalyst',
      contracts: 'Key contracts',
      stages: {
        producer: 'Producer',
        developer: 'Developer',
        exploration: 'Exploration',
      },
      profits: {
        profitable: 'Profitable',
        transitioning: 'Transitioning',
        notYet: 'Not yet',
      },
      dividends: {
        none: 'None',
        paying: 'Paying',
        variable: 'Variable',
      },
      risks: {
        low: 'Low',
        moderate: 'Moderate',
        high: 'High',
      },
    },
  },
  quarterlyDy: {
    title: 'Quarterly DY',
    subtitle: 'Quarterly dividend payers — US Aristocrats, Canadian banks & energy, and UK/European blue-chips ranked by yield.',
    updatedAt: 'Updated {date}',
    refreshesWeekly: 'Refreshes weekly',
    loading: 'Loading Quarterly DY picks...',
    empty: 'No Quarterly DY picks yet. The list refreshes weekly — check back soon.',
    error: 'Failed to load Quarterly DY picks.',
    searchPlaceholder: 'Search by name or ticker…',
    yieldFilterLabel: 'Minimum yield',
    yieldFilterAll: 'All yields',
    sortLabel: 'Sort by',
    sortYieldDesc: 'Yield (high → low)',
    sortYieldAsc: 'Yield (low → high)',
    sortMarketCap: 'Market cap',
    sortRisk: 'Risk',
    yieldLabel: 'Yield',
    annualDividend: 'Annual',
    perPaymentLabel: 'Per quarter',
    paymentsPerYear: '{n}× / year',
    marketCap: 'Market cap',
    price: 'Price',
    sector: 'Sector',
    exchange: 'Exchange',
    industry: 'Industry',
    nextExDividendOn: 'Next ex-div {date}',
    nextExDividendIn: 'Next ex-div in {days} days',
    nextExDividendToday: 'Goes ex-dividend today',
    nextExDividendUnknown: 'Next ex-div date not announced',
    nextPaymentOn: 'Pays {date}',
    lastDividendOn: 'Last paid {date}',
    nextExDividendLabel: 'Next ex-dividend',
    nextPaymentLabel: 'Next payment',
    lastDividendLabel: 'Last paid',
    pickCount: '{count} picks',
    riskTitle: 'Risk',
    riskLow: 'Low',
    riskMedium: 'Medium',
    riskHigh: 'High',
    riskLowDesc: 'S&P 500 Dividend Aristocrats and regulated utilities with 25+ consecutive years of dividend growth.',
    riskMediumDesc: 'Dividend Achievers, major banks, energy pipelines and REITs — strong franchises with moderate macro sensitivity.',
    riskHighDesc: 'Higher-risk quarterly payers — verify payout sustainability before investing.',
    assetAristocrat: 'Dividend Aristocrat',
    assetAchiever: 'Dividend Achiever',
    assetUtility: 'Utility',
    assetBank: 'Bank',
    assetEnergy: 'Energy',
    assetReit: 'REIT',
    backToList: 'Back to Quarterly DY',
    priceHistoryTitle: 'Price evolution (5 years)',
    priceHistorySince: 'Since {date}',
    priceLabel: 'Price',
    dividendMarkerLabel: 'Dividend',
    dividendTooltipAmount: 'Dividend: {amount}',
    distributionsTitle: 'Distributions (up to last 5 years)',
    distributionsYearTotal: '{count} payments · {total}',
    distributionsEmpty: 'No distributions on record.',
    notFound: 'Pick not found in the current list.',
  },
  monthlyDy: {
    title: 'Monthly DY',
    subtitle: 'Monthly-paying dividend instruments — ranked by current yield.',
    updatedAt: 'Updated {date}',
    refreshesWeekly: 'Refreshes weekly',
    loading: 'Loading Monthly DY picks...',
    empty: 'No Monthly DY picks yet. The list refreshes weekly — check back soon.',
    error: 'Failed to load Monthly DY picks.',
    searchPlaceholder: 'Search by name or ticker…',
    yieldFilterLabel: 'Minimum yield',
    yieldFilterAll: 'All yields',
    sortLabel: 'Sort by',
    sortYieldDesc: 'Yield (high → low)',
    sortYieldAsc: 'Yield (low → high)',
    sortMarketCap: 'Market cap',
    sortRisk: 'Risk',
    yieldLabel: 'Yield',
    annualDividend: 'Annual',
    perPaymentLabel: 'Per payment',
    paymentsPerYear: '{n}× / year',
    marketCap: 'Market cap',
    price: 'Price',
    sector: 'Sector',
    exchange: 'Exchange',
    industry: 'Industry',
    nextExDividendOn: 'Next ex-div {date}',
    nextExDividendIn: 'Next ex-div in {days} days',
    nextExDividendToday: 'Goes ex-dividend today',
    nextExDividendUnknown: 'Next ex-div date not announced',
    nextPaymentOn: 'Pays {date}',
    lastDividendOn: 'Last paid {date}',
    pickCount: '{count} picks',
    riskTitle: 'Risk',
    riskLow: 'Low',
    riskMedium: 'Medium',
    riskHigh: 'High',
    riskLowDesc: 'Established equity REITs and dividend stocks with stable cash flows.',
    riskMediumDesc: 'BDCs, midstream energy and similar — leveraged business models with cash-flow sensitivity.',
    riskHighDesc: 'Mortgage REITs, leveraged or distribution-volatile names — cuts and capital erosion are common.',
    assetEquityReit: 'Equity REIT',
    assetMortgageReit: 'Mortgage REIT',
    assetBdc: 'BDC',
    assetEnergyInfra: 'Energy infrastructure',
    assetStock: 'Stock',
    assetEtf: 'ETF',
    backToList: 'Back to Monthly DY',
    priceHistoryTitle: 'Price evolution (5 years)',
    priceHistorySince: 'Since {date}',
    priceLabel: 'Price',
    dividendMarkerLabel: 'Dividend',
    dividendTooltipAmount: 'Dividend: {amount}',
    distributionsTitle: 'Distributions (up to last 5 years)',
    distributionsYearTotal: '{count} payments · {total}',
    distributionsEmpty: 'No distributions on record.',
    keyFactsTitle: 'Key facts',
    expenseRatio: 'Expense ratio',
    notFound: 'Pick not found in the current list.',
  },
  follow: {
    listTitle: 'Followed companies',
    listSubtitle: 'Everything you are tracking, in one place.',
    loginRequired: 'Log in to follow companies and keep them here.',
    loading: 'Loading your list...',
    emptyTitle: 'No followed companies yet',
    emptyBody: 'Follow companies from the AI Picks or Dividends pages to see them here.',
    sourceAi: 'AI Pick',
    sourceDividend: 'Dividend',
    sourceMonthlyDy: 'Monthly DY',
    sourceQuarterlyDy: 'Quarterly DY',
    filterAll: 'All',
    followedOn: 'Followed on {date}',
    follow: 'Follow',
    unfollow: 'Unfollow',
    following: 'Following',
    nextExDiv: 'Next ex-div',
    beatsInflation: 'Beats inflation +{amount}%',
    losesInflation: 'Loses to inflation −{amount}%',
    matchesInflation: 'Matches inflation',
    liveDataRefreshing: 'Live data refreshing…',
  },
  settings: {
    title: 'Settings',
    subtitle: 'Hi {name} — adjust how the picks are shown to you.',
    save: 'Save',
    saved: 'Saved',
    clear: 'Clear',
    error: "Couldn't save. Try again.",
    taxCountry: {
      title: 'Tax residence',
      help: 'Used to compare each pick’s nominal yield against your country’s inflation, so you know whether you’re beating or losing to inflation.',
      label: 'Country where you declare taxes',
      placeholder: '— Select your tax country —',
      tooltip: 'Pick the country whose inflation rate matters to you. Only EU-27 countries are supported today (the ones we have HICP data for).',
    },
    wht: {
      title: 'Withholding tax on dividends',
      helpForUser: 'How much each source country withholds from dividends paid to a {country} tax resident — by default, and after filing the relevant tax-treaty paperwork.',
      helpNoUser: 'Save your tax country above to see exactly how much each source country withholds from dividends paid to you.',
      disclaimer: 'Educational only — verify with your tax advisor or broker before relying on these numbers. Treaty rates assume retail status and proper paperwork; bilateral treaties may change.',
      searchLabel: 'Search source country',
      searchPlaceholder: 'Search country (e.g. Germany, Switzerland, US)…',
      statutory: 'Statutory',
      treaty: 'With treaty',
      colCountry: 'Source country',
      colMechanism: 'Mechanism',
      colNotes: 'Notes',
      appliedAtSource: 'Applied at source',
      requiresReclaim: 'Reclaim required',
      noTreatyBenefit: 'No treaty benefit',
      noResults: 'No countries match “{query}”.',
      reviewedOn: 'Reviewed {date}. Always confirm with primary tax-authority sources.',
      setCountryFirst: 'Set your tax country above to see country-specific withholding rates.',
    },
  },
}
