export default {
  nav: {
    dashboard: 'Painel',
    interestRate: 'Taxa de Juros',
    inflation: 'Inflação',
    dividends: 'Dividendos',
    aiRecommendation: 'Picks',
    monthlyDy: 'Monthly DY',
    quarterlyDy: 'Quarterly DY',
    followed: 'Seguindo',
    minerals: 'Minerais',
  },
  dashboard: {
    title: 'Visão Geral',
    subtitle: 'Seu painel financeiro para vencer a inflação',
    ecbRate: 'Taxa BCE Depósito',
    depositFacility: 'Facilidade de Depósito',
    countriesTracked: 'Países Monitorados',
    europeanCountries: 'países europeus',
    stocksAboveRate: 'Ações Acima do BCE',
    beatingEcbRate: 'Acima da taxa BCE',
    beatingInflation: 'Vencendo a Inflação',
    beatingLocalInflation: 'Vencendo a inflação local',
    topOpportunities: 'Melhores Oportunidades de Dividendos',
    upcomingExDividends: 'Próximos Ex-Dividendos',
    upcomingExDividendsSubtitle: 'Empresas dos Picks de IA e Monthly DY com datas ex-dividendo anunciadas e pagamentos futuros.',
    next7Days: 'Próximos 7 dias',
    next30Days: 'Próximos 30 dias',
    exDividendDate: 'Ex-dividendo',
    paymentValue: 'Valor pago',
    dividendYield: 'DY',
    paymentsPerYear: 'Pagamentos',
    paymentFrequency: '{count}x / ano',
    paymentTomorrow: 'Paga amanhã',
    paymentInDays: 'Paga em {days} dias',
    paymentDate: 'Paga {date}',
    sourceMonthlyDy: 'Monthly DY',
    sourceQuarterlyDy: 'Quarterly DY',
    sourceAiPicks: 'Picks IA',
    viewMonthlyDy: 'Ver Monthly DY',
    noUpcomingExDividends: 'Nenhum pagamento ex-dividendo anunciado nos próximos 30 dias.',
    highestInflation: 'Países com Maior Inflação',
    lowestInflation: 'Países com Menor Inflação',
    viewAll: 'Ver todos',
    trendRising: 'Subindo',
    trendFalling: 'Caindo',
    trendStable: 'Estável',
  },
  interestRate: {
    title: 'Taxas de Juros do BCE',
    subtitle: 'Taxas de juros chave do Banco Central Europeu',
    deposit: 'Facilidade de Depósito',
    main: 'Refinanciamento Principal',
    marginal: 'Empréstimo Marginal',
    rateType: 'Tipo de Taxa',
    value: 'Valor',
    effectiveDate: 'Data de Vigência',
    loading: 'Carregando taxas de juros...',
    whatAreRates: 'O que são as taxas do BCE?',
    ratesDescription:
      'O BCE define três taxas de juros chave. A taxa de depósito é o que os bancos recebem por depositar dinheiro no BCE durante a noite. A taxa de refinanciamento principal é o custo para os bancos pedirem emprestado ao BCE por uma semana. A taxa de empréstimo marginal é o custo do empréstimo overnight no BCE. Essas taxas influenciam diretamente a inflação e os custos de crédito na Zona Euro.',
  },
  inflation: {
    title: 'Inflação por País',
    subtitle: 'Taxa de inflação anual IPC nos países europeus',
    country: 'País',
    rate: 'Taxa',
    trend: 'Tendência',
    vsEcb: 'vs Taxa BCE',
    date: 'Data',
    searchPlaceholder: 'Buscar país...',
    filterAll: 'Todos',
    filterHigh: 'Alta (>4%)',
    filterMedium: 'Média (2–4%)',
    filterLow: 'Baixa (<2%)',
    loading: 'Carregando dados de inflação...',
    dataLag: 'Exibindo dados de {date} — a API do Eurostat publica com ~3 meses de atraso.',
    noResults: 'Nenhum país encontrado.',
  },
  inflationCountry: {
    back: 'Voltar para todos os países',
    subtitle: 'Inflação HICP mensal — variação anual',
    loading: 'Carregando dados históricos...',
    error: 'Falha ao carregar o histórico de inflação.',
    latest: 'Mais recente',
    average: 'Média 10 anos',
    tenYears: 'Últimos 10 anos',
    peak: 'Pico',
    low: 'Mínimo',
    chartTitle: 'Taxa de inflação nos últimos 10 anos (%)',
    debtLabel: 'Dívida Pública',
    debtOfGdp: 'do PIB',
    debtSince: 'desde {year}',
    aiInsightsTitle: 'Análise Econômica por IA',
    prioritiesTitle: 'Principais Prioridades',
    problemsTitle: 'Principais Problemas',
    opportunitiesTitle: 'Oportunidades',
  },
  dividendCompany: {
    back: 'Voltar para dividendos',
    price: 'Preço',
    yieldLabel: 'Dividend Yield',
    annualDividend: 'Dividendo Anual',
    marketCap: 'Cap. de Mercado',
    beta: 'Beta',
    betaLow: 'abaixo do mercado',
    betaHigh: 'acima do mercado',
    weekHigh: 'Máxima 52 semanas',
    weekLow: 'Mínima 52 semanas',
    chartTitle: 'Yield vs Referências',
    ecbRateLabel: 'Taxa BCE',
    inflationLabel: 'Inflação Local',
    about: 'Sobre',
    ceoLabel: 'CEO',
    websiteLabel: 'Visitar site',
    proTitle: 'Por que considerar',
    conTitle: 'Risco a observar',
    loading: 'Carregando dados da empresa...',
    error: 'Falha ao carregar dados da empresa.',
    pro: {
      beatsInflation: 'Dividend yield de {yield}% supera a inflação local ({inflation}%), protegendo seu poder de compra em termos reais.',
      beatsEcb: 'Yield de {yield}% supera a taxa BCE de {rate}%, sendo mais atrativo que poupança sem risco.',
      stableSector: 'Empresas do setor {sector} são historicamente das mais confiáveis pagadoras de dividendos, com fluxo de caixa estável.',
      lowBeta: 'Beta baixo de {beta} indica volatilidade de preço abaixo da média — menos turbulento que o mercado em geral.',
      established: 'Empresa de grande capitalização com market cap de {cap} e histórico consolidado de retorno de capital aos acionistas.',
    },
    con: {
      highYield: 'Um yield acima de 8% pode indicar risco elevado ou corte iminente de dividendo — verifique o payout ratio e a cobertura de lucros.',
      cyclical: '{sector} é um setor cíclico — os dividendos tendem a flutuar e podem ser reduzidos durante recessões.',
      highBeta: 'Beta de {beta} indica volatilidade acima da média — a ação pode oscilar bastante com os movimentos do mercado.',
      belowEcb: 'O yield atual está abaixo da taxa BCE de {rate}% — o dividendo ainda não supera aplicações sem risco.',
      concentration: 'Exposição a uma única ação concentra risco. Avalie se essa posição está diversificada por setores e geografias na sua carteira.',
    },
  },
  dividends: {
    title: 'Rastreador de Dividendos',
    subtitle: 'Ações europeias com alto dividend yield — encontre o que vence a inflação',
    company: 'Empresa',
    country: 'País',
    yield: 'Yield',
    sector: 'Setor',
    exchange: 'Bolsa',
    status: 'Status',
    ecbRateLabel: 'Taxa BCE',
    legendBeatsInflation: 'Vence a inflação local',
    legendBeatsEcb: 'Vence a taxa BCE',
    beatsEcb: 'Vence a Taxa BCE',
    beatsInflation: 'Vence a Inflação',
    searchPlaceholder: 'Buscar empresa ou ticker...',
    filterAll: 'Todos',
    filterBeatsRate: 'Vence a Taxa BCE',
    filterBeatsInflation: 'Vence a Inflação',
    loading: 'Carregando dados de dividendos...',
    noResults: 'Nenhuma ação encontrada.',
  },
  aiRecommendation: {
    title: 'Picks de IA para Dividendos',
    subtitle: 'Top 10 ações europeias de alto rendimento — curados por IA, atualizados diariamente',
    updatedAt: 'Atualizado em {date}',
    refreshesDaily: 'Atualiza diariamente às 8:00 UTC',
    loading: 'Carregando recomendações de IA...',
    empty: 'Sem recomendações ainda. A IA gera novos picks uma vez por dia — volte em breve.',
    error: 'Falha ao carregar recomendações de IA.',
    proTitle: 'Por que investir',
    conTitle: 'Risco principal',
    statusBullish: 'Otimista',
    statusNeutral: 'Neutro',
    statusBearish: 'Pessimista',
    yieldHistory: 'Histórico de Yield (5 anos)',
    dividendsPerYear: 'Dividendos por Ano',
    yieldLabel: 'Dividend Yield',
    price: 'Preço',
    annualDiv: 'Dividendo Anual',
    marketCap: 'Cap. de Mercado',
    kicker: 'IA semanal · {date}',
    kickerNoDate: 'IA semanal',
    headlinePre: 'Europa de alto yield,',
    headlineAccent: 'curada',
    editorialSubtitle: 'Dez pagadoras de dividendos europeias, ordenadas por yield. Aqui está a tese da IA, não a planilha.',
    pickOfTheWeek: 'Escolha da semana',
    rankNumber: 'Posição #{rank}',
    alsoRanking: 'Também no ranking',
    morePicks: 'mais {count} picks',
    readFullAnalysis: 'Ver análise completa',
    backToPicks: 'Voltar para os Picks',
    companyNotFound: 'Empresa não encontrada nas escolhas atuais.',
    loadingHistory: 'Carregando histórico...',
    evolutionTitle: 'Evolução de yield e dividendos',
    yearlyYieldLabel: 'yield de {yield}%',
    perShareYear: 'por ação / ano',
    paymentDates: 'Datas de pagamento',
    paidOn: 'Pago em {date}',
    realYieldLabel: 'Yield real',
    realYieldHint: '{nominal}% yield − {inflation}% HICP ({country})',
    realYieldHintNeg: 'A inflação supera este yield em {country}',
    realYieldNoData: 'HICP indisponível para {country} — exibindo yield nominal',
    inflationLabel: 'Inflação',
    nominalYieldLabel: 'Yield nominal',
    hicpRate: 'HICP {rate}%',
    hicpAsOf: 'HICP em {date}',
    inflationBeatTitle: 'vs. sua inflação',
    inflationBeatBeats: 'Vence sua inflação em {amount}%',
    inflationBeatMatches: 'Praticamente empata com sua inflação',
    inflationBeatLoses: 'Perde para sua inflação em {amount}%',
    inflationBeatShort: 'Vence a inflação +{amount}%',
    inflationBeatShortNeg: 'Perde para a inflação −{amount}%',
    inflationBeatShortMatch: 'Empata com a inflação',
    inflationBeatHint: 'Yield nominal {nominal}% − HICP {inflation}% em {country}',
    inflationCtaTitle: 'Veja como cada pick vence sua inflação',
    inflationCtaBody: 'Defina seu país fiscal nas Configurações e mostraremos, por pick, se você está ganhando ou perdendo contra a inflação local.',
    inflationCtaLink: 'Abrir configurações',
    tierLegendTitle: 'Estratégia',
    tierLegendBody: 'Cada pick passou em um de três filtros de qualidade. Conservador é o mais rigoroso; Permissivo é o mais flexível.',
    tierConservative: 'Conservador',
    tierModerate: 'Moderado',
    tierPermissive: 'Permissivo',
    tierConservativeShort: 'Estratégia conservadora',
    tierModerateShort: 'Estratégia moderada',
    tierPermissiveShort: 'Estratégia permissiva',
    tierConservativeDesc: 'Passa pelo nosso filtro mais rigoroso — sustentabilidade, crescimento e rentabilidade fortes em todos os pontos.',
    tierModerateDesc: 'Passa pelo nosso filtro padrão — fundamentos sólidos com alguma variabilidade tolerada.',
    tierPermissiveDesc: 'Passa pelo nosso filtro mais flexível — alto yield com métricas fracas ou incompletas; verifique com atenção.',
    disclaimer: {
      complianceLead: 'Apenas informativo · não é aconselhamento financeiro',
      complianceBody: 'Conteúdo gerado por IA. Os valores podem estar incorretos ou desatualizados. Verifique em fontes oficiais e consulte um assessor licenciado antes de qualquer decisão de investimento.',
      aboutTitle: 'Sobre estes picks',
      aboutMethodologyTitle: 'Metodologia',
      aboutMethodology: 'A lista é gerada diariamente pelo Google Gemini, um modelo de linguagem por IA, a partir de um prompt no estilo de analista financeiro que busca pagadoras de dividendos europeias de grande capitalização com yield acima de 2× a atual taxa de depósito do BCE. Os valores numéricos (preço, yield, capitalização de mercado, histórico de dividendos) são gerados por IA e podem estar incorretos ou desatualizados — verifique em fontes oficiais (site de Relações com Investidores da empresa, sua corretora, ou dados oficiais da bolsa) antes de qualquer decisão.',
      aboutNotAdviceTitle: 'Não é aconselhamento financeiro',
      aboutNotAdvice: 'Nada aqui é uma recomendação de investimento personalizada ou aconselhamento. Resultados passados não garantem resultados futuros. Consulte um assessor financeiro licenciado antes de qualquer decisão de investimento.',
      aboutConflictsTitle: 'Conflitos de interesse',
      aboutConflicts: 'O publicador de winflation.eu pode possuir posições em algumas das ações listadas.',
    },
  },
  footer: {
    disclaimer: 'winflation.eu é uma ferramenta informativa e educacional. Nada neste site constitui aconselhamento financeiro, de investimento ou fiscal. Os dados podem estar incorretos ou desatualizados — verifique em fontes oficiais antes de qualquer decisão de investimento.',
  },
  auth: {
    or: 'ou',
    email: 'E-mail',
    password: 'Senha',
    name: 'Nome completo',
    menu: {
      login: 'Entrar',
      logout: 'Sair',
      settings: 'Configurações',
    },
    login: {
      title: 'Bem-vindo de volta',
      subtitle: 'Entre para acompanhar as empresas que você segue.',
      google: 'Continuar com Google',
      submit: 'Entrar',
      noAccount: 'Ainda não tem conta?',
      signUp: 'Cadastre-se',
    },
    register: {
      title: 'Crie sua conta',
      subtitle: 'Comece a seguir as ações europeias de dividendos que importam pra você.',
      google: 'Cadastrar com Google',
      submit: 'Criar conta',
      passwordHint: 'Mínimo de 6 caracteres.',
      haveAccount: 'Já tem uma conta?',
      signIn: 'Entrar',
    },
    errors: {
      invalidEmail: 'Esse e-mail não é válido.',
      userDisabled: 'Esta conta foi desativada.',
      invalidCredentials: 'E-mail ou senha incorretos.',
      emailInUse: 'Já existe uma conta com esse e-mail.',
      weakPassword: 'A senha deve ter pelo menos 6 caracteres.',
      popupClosed: 'Janela de login fechada antes de concluir.',
      network: 'Erro de rede — verifique sua conexão.',
      tooManyRequests: 'Muitas tentativas. Tente novamente em alguns minutos.',
      generic: 'Algo deu errado. Tente de novo.',
    },
  },
  minerals: {
    title: 'Minerais Raros',
    subtitle: 'Onde o mundo extrai os metais que alimentam veículos elétricos, turbinas eólicas e hardware de IA',
    loading: 'Carregando dados de minerais...',
    dataAsOf: 'Dados de {date}',
    warning: {
      title: 'Aviso-chave ao investidor — tomadores de preço',
      body: 'Estas empresas são tomadoras de preço. As ações oscilam fortemente com o preço global do NdPr (neodímio-praseodímio). Se a China inundar o mercado para derrubar preços, as margens encolhem imediatamente. Em 2026, governos ocidentais fornecem subsídios e preços-piso para manter viável a oferta fora da China.',
    },
    mapTitle: 'Reservas e operações globais',
    mapHint: 'Clique em um marcador para ver os minerais e empresas ativas no país.',
    countriesTitle: 'Países-chave',
    countriesHint: 'Ordenados pelo número de minerais críticos produzidos.',
    mineralsTitle: 'Minerais críticos para 2026',
    usedFor: 'Usado para',
    viewCountry: 'Ver detalhes do país',
    companiesCount: '{count} empresas | {count} empresa | {count} empresas',
    why: {
      title: 'Por que minerais raros importam para investidores',
      body: 'A China ainda controla ~60% da produção global de terras raras e um domínio ainda maior sobre óxidos refinados e fabricação de ímãs. À medida que veículos elétricos, turbinas eólicas, robótica e defesa crescem, governos ocidentais estão financiando novas minas e refinarias fora da China — criando uma rara oportunidade de investimento em um mercado estruturalmente monopolizado por duas décadas.',
    },
    category: {
      all: 'Todos',
      magnet: 'Metais de ímã',
      battery: 'Bateria',
      other: 'Outros',
    },
    entries: {
      Nd: {
        name: 'Neodímio',
        description: 'Parte do "par de potência" com o Praseodímio, forma a espinha dorsal dos ímãs permanentes em motores elétricos.',
        use: 'Ímãs permanentes para motores de VE, turbinas eólicas e eletrônicos de consumo.',
      },
      Pr: {
        name: 'Praseodímio',
        description: 'Combinado com Neodímio em ligas NdPr — juntos dominam o mercado de ímãs de alta performance.',
        use: 'Ímãs permanentes de alta resistência, ligas aeroespaciais.',
      },
      Dy: {
        name: 'Disprósio',
        description: 'Terra rara "pesada" adicionada aos ímãs para que mantenham força em altas temperaturas.',
        use: 'Ímãs de alta temperatura para VEs e sistemas de defesa.',
      },
      Tb: {
        name: 'Térbio',
        description: 'Outra terra rara pesada que melhora a coercividade do ímã — crítica para motores de alta performance.',
        use: 'Ímãs resistentes ao calor, fósforos em displays.',
      },
      Li: {
        name: 'Lítio',
        description: 'O metal mais leve e a base da química moderna de baterias — não é tecnicamente uma terra rara, mas agrupado como "mineral raro".',
        use: 'Baterias de íons de lítio para VEs e armazenamento em rede.',
      },
      Co: {
        name: 'Cobalto',
        description: 'Estabiliza cátodos em baterias de íons de lítio, prolongando vida útil e segurança.',
        use: 'Cátodos de bateria (químicas NMC / NCA), superligas.',
      },
      Graphite: {
        name: 'Grafite',
        description: 'Essencial para ânodos de bateria. A China domina globalmente a etapa de refino do grafite "esférico".',
        use: 'Ânodos em praticamente todas as baterias de íons de lítio.',
      },
      La: {
        name: 'Lantânio',
        description: 'Terra rara leve usada em catalisadores e vidros ópticos especiais.',
        use: 'Catalisadores de refinaria, lentes de câmera, baterias NiMH.',
      },
      Ce: {
        name: 'Cério',
        description: 'A terra rara mais abundante, frequentemente co-produzida com Lantânio a partir de minérios de monazita.',
        use: 'Polimento de vidro, catalisadores automotivos, ligas metálicas.',
      },
      Monazite: {
        name: 'Monazita',
        description: 'Areia mineral contendo terras raras, também com tório. A Índia tem enormes reservas em suas areias de praia.',
        use: 'Matéria-prima para concentrados mistos de terras raras.',
      },
    },
    countries: {
      CN: {
        name: 'China',
        status: 'Produtor dominante dos 17 elementos de terras raras e grafite; controla preços e refino global.',
        companies: {
          chinaRareEarthGroup: 'Mega-empresa estatal criada pela fusão de várias empresas menores para controlar o mercado doméstico.',
          shengheResources: 'Grande player global que também investe em projetos no exterior (EUA, Groenlândia).',
        },
      },
      VN: {
        name: 'Vietnã',
        status: 'Segundas maiores reservas do mundo (~22Mt); produção crescendo rapidamente com apoio ocidental.',
        companies: {
          vtre: 'Trabalhando com empresas australianas e coreanas para reativar a mina Dong Pao, uma das maiores do mundo.',
          blackstone: 'Explorando ativamente projetos de níquel e terras raras associadas no distrito de Ta Khoa.',
        },
      },
      BR: {
        name: 'Brasil',
        status: 'Depósitos massivos em Minas Gerais; recentemente se tornou fornecedor "Tier 1" para os EUA.',
        companies: {
          serraVerde: 'Opera a mina Pela Ema, uma das únicas fontes fora da Ásia para os quatro metais de ímã (Nd, Pr, Tb, Dy).',
          appia: 'Explorando ativamente o projeto de alta qualidade Cachoeira de Paita.',
        },
      },
      AU: {
        name: 'Austrália',
        status: 'Principal fonte não-chinesa de terras raras leves.',
        companies: {
          lynas: {
            description: 'Maior produtor de terras raras fora da China — opera a mina Mount Weld e uma planta de separação na Malásia.',
            catalyst: 'Expansão da planta de processamento na Malásia; escalando produção de NdPr para atender demanda aliada de longo prazo.',
            contracts: 'Contratos estratégicos de fornecimento com o Departamento de Defesa dos EUA e parceiros industriais japoneses.',
          },
          iluka: 'Construindo a primeira refinaria totalmente integrada de terras raras da Austrália.',
        },
      },
      US: {
        name: 'Estados Unidos',
        status: 'Escalando produção doméstica de ímãs para acompanhar sua extração.',
        companies: {
          mpMaterials: {
            description: 'Opera a mina Mountain Pass na Califórnia; avançando para produção "mina-a-ímã" para contornar fábricas chinesas.',
            catalyst: 'Ramp-up da planta de ímãs "10X" no Texas (US$ 200M) — produção totalmente integrada do minério ao ímã.',
            contracts: 'Acordo de offtake relevante com a General Motors para fornecer ímãs permanentes para motores de VE.',
          },
          usaRareEarth: {
            description: 'Desenvolvendo o projeto Round Top no Texas; em abril de 2026 adquiriu a mina Pela Ema (Brasil) por US$ 2,8 bi — estratégia "mina-a-ímã" alimentando instalações nos EUA.',
            catalyst: 'Integração da aquisição brasileira (Pela Ema); produção prevista de 6.400 toneladas de óxidos de terras raras por ano até 2027.',
            contracts: 'Cadeia verticalizada: minério brasileiro abastece processamento e fabricação de ímãs nos EUA — contornando a refinação chinesa.',
          },
        },
      },
      IN: {
        name: 'Índia',
        status: 'Depósitos massivos de areias de praia (monazita); atualmente focado em uso doméstico de alta tecnologia.',
        companies: {},
      },
      CA: {
        name: 'Canadá',
        status: 'Expansão rápida nos Territórios do Noroeste e Quebec.',
        companies: {
          vitalMetals: 'Primeiro produtor de terras raras do Canadá, focado no projeto Nechalacho.',
          neo: 'Empresa com sede no Canadá operando plantas de processamento globalmente (incluindo Estônia) para produzir pós e ímãs de alto valor.',
        },
      },
      ZA: {
        name: 'África do Sul',
        status: 'Lar de Steenkampskraal — o depósito de terras raras de maior grau do mundo (~14,5% de óxidos vs. 1-2% típico em outros lugares).',
        companies: {
          smm: 'Operadora da mina Steenkampskraal, executando um plano de seis fases para minerar, processar e separar os metais domesticamente.',
        },
      },
      MW: {
        name: 'Malauí',
        status: 'Player em rápida ascensão em metais para ímãs; o projeto Songwe Hill está entre os poucos no mundo a atingir viabilidade definitiva.',
        companies: {
          mkango: {
            description: 'Construindo um pipeline "mina-ao-mercado" a partir de Songwe Hill — mineração no Malauí, separação na Polônia, venda direta a montadoras europeias.',
            catalyst: 'Estudo de Viabilidade Definitiva concluído em março/2026. Construção prevista para 2027; primeira produção em 2029. Fechamento do financiamento é o catalisador de curto prazo.',
            contracts: 'Integração vertical: concentrado de Songwe Hill segue para planta de separação dedicada na Polônia para offtake de OEMs europeias.',
          },
        },
      },
      NA: {
        name: 'Namíbia',
        status: 'Reservas massivas e uma rara fonte de terras raras pesadas (Dy, Tb) necessárias para ímãs de alta temperatura.',
        companies: {
          bannerman: 'Principalmente desenvolvedora de urânio (Projeto Etango), mas apoia infraestrutura-chave para o setor de minerais raros.',
          ncm: 'Desenvolvendo o projeto Lofdal, especificamente rico em terras raras pesadas para ímãs de alta temperatura.',
        },
      },
      BI: {
        name: 'Burundi',
        status: 'Lar de Gakara — historicamente o concentrado de terras raras de maior pureza do mundo.',
        companies: {
          rainbow: {
            description: 'Operadora do projeto Gakara; ativo de alto nível apesar de atritos políticos com o governo sobre divisão de lucros. Também desenvolve projeto de baixo custo de extração de fosfogesso na África do Sul.',
            catalyst: 'Piloto bem-sucedido de extração a partir de pilhas de rejeito (fosfogesso) — CAPEX muito menor do que mineração profunda convencional.',
            contracts: 'US$ 50M garantidos junto à U.S. International Development Finance Corporation (DFC) — forte apoio político ocidental.',
          },
        },
      },
      CD: {
        name: 'RD Congo',
        status: 'Produz mais de 70% do cobalto mundial — o estabilizador crítico em cátodos de baterias de íons de lítio.',
        companies: {
          glencore: 'Grande operadora de minas de cobre-cobalto em Katanga — uma das maiores produtoras de cobalto do mundo.',
          cmoc: 'Operadora apoiada pela China de Tenke Fungurume e Kisanfu — entre as maiores minas de cobalto do mundo.',
        },
      },
      ML: {
        name: 'Mali',
        status: 'Tornou-se grande produtor de lítio no final de 2025 / início de 2026 quando o projeto Goulamina entrou em produção.',
        companies: {
          ganfeng: 'Operadora do projeto de lítio Goulamina — um dos maiores depósitos de espodumênio da África.',
          leoLithium: 'Parceira de desenvolvimento em Goulamina, um projeto de lítio-chave fornecendo ao mercado de baterias de VE.',
        },
      },
    },
  },
  mineralsCountry: {
    back: 'Voltar para minerais',
    notFound: 'País não encontrado no conjunto de dados.',
    mineralsTitle: 'Minerais encontrados aqui',
    companiesTitle: 'Empresas operando no local',
    companiesEmpty: 'Nenhum operador público relevante rastreado neste país ainda.',
    mineralsCount: '{count} minerais | {count} mineral | {count} minerais',
    companiesCount: '{count} empresas | {count} empresa | {count} empresas',
    financials: {
      profitability: 'Lucratividade',
      dividend: 'Dividendo',
      mineLife: 'Vida útil',
      risk: 'Risco',
      revenueYoY: 'Receita YoY',
      netIncome: 'Lucro líquido',
      primaryCatalyst: 'Catalisador principal',
      contracts: 'Contratos-chave',
      stages: {
        producer: 'Produtora',
        developer: 'Em desenvolvimento',
        exploration: 'Exploração',
      },
      profits: {
        profitable: 'Lucrativa',
        transitioning: 'Em transição',
        notYet: 'Ainda não',
      },
      dividends: {
        none: 'Nenhum',
        paying: 'Paga',
        variable: 'Variável',
      },
      risks: {
        low: 'Baixo',
        moderate: 'Moderado',
        high: 'Alto',
      },
    },
  },
  quarterlyDy: {
    title: 'Quarterly DY',
    subtitle: 'Pagadores trimestrais — Aristocratas dos EUA, bancos e energia canadenses e blue-chips europeus ordenados por yield.',
    updatedAt: 'Atualizado em {date}',
    refreshesWeekly: 'Atualiza semanalmente',
    loading: 'Carregando picks de Quarterly DY...',
    empty: 'Sem picks ainda. A lista é atualizada semanalmente — volte em breve.',
    error: 'Falha ao carregar picks de Quarterly DY.',
    searchPlaceholder: 'Buscar por nome ou ticker…',
    yieldFilterLabel: 'Yield mínimo',
    yieldFilterAll: 'Todos os yields',
    sortLabel: 'Ordenar por',
    sortYieldDesc: 'Yield (maior → menor)',
    sortYieldAsc: 'Yield (menor → maior)',
    sortMarketCap: 'Cap. de mercado',
    sortRisk: 'Risco',
    yieldLabel: 'Yield',
    annualDividend: 'Anual',
    perPaymentLabel: 'Por trimestre',
    paymentsPerYear: '{n}× / ano',
    marketCap: 'Cap. mercado',
    price: 'Preço',
    sector: 'Setor',
    exchange: 'Bolsa',
    industry: 'Indústria',
    nextExDividendOn: 'Próx. ex-div {date}',
    nextExDividendIn: 'Próx. ex-div em {days} dias',
    nextExDividendToday: 'Vai ex-dividendo hoje',
    nextExDividendUnknown: 'Próx. ex-div não anunciada',
    nextPaymentOn: 'Pagamento {date}',
    lastDividendOn: 'Último pagamento {date}',
    nextExDividendLabel: 'Próximo ex-dividendo',
    nextPaymentLabel: 'Próximo pagamento',
    lastDividendLabel: 'Último pagamento',
    pickCount: '{count} picks',
    riskTitle: 'Risco',
    riskLow: 'Baixo',
    riskMedium: 'Médio',
    riskHigh: 'Alto',
    riskLowDesc: 'Aristocratas do S&P 500 e utilities reguladas com 25+ anos consecutivos de crescimento de dividendos.',
    riskMediumDesc: 'Dividend Achievers, grandes bancos, energia e REITs — franquias sólidas com sensibilidade macroeconômica moderada.',
    riskHighDesc: 'Pagadores trimestrais de maior risco — verifique a sustentabilidade do payout antes de investir.',
    assetAristocrat: 'Aristocrata de Dividendos',
    assetAchiever: 'Dividend Achiever',
    assetUtility: 'Utility',
    assetBank: 'Banco',
    assetEnergy: 'Energia',
    assetReit: 'REIT',
    backToList: 'Voltar para Quarterly DY',
    priceHistoryTitle: 'Evolução do preço (5 anos)',
    priceHistorySince: 'Desde {date}',
    priceLabel: 'Preço',
    dividendMarkerLabel: 'Dividendo',
    dividendTooltipAmount: 'Dividendo: {amount}',
    distributionsTitle: 'Pagamentos (até os últimos 5 anos)',
    distributionsYearTotal: '{count} pagamentos · {total}',
    distributionsEmpty: 'Nenhum pagamento registrado.',
    notFound: 'Pick não encontrado na lista atual.',
  },
  monthlyDy: {
    title: 'Monthly DY',
    subtitle: 'Investimentos com pagamento mensal — ordenados pelo yield atual.',
    updatedAt: 'Atualizado em {date}',
    refreshesWeekly: 'Atualiza semanalmente',
    loading: 'Carregando picks de Monthly DY...',
    empty: 'Sem picks ainda. A lista é atualizada semanalmente — volte em breve.',
    error: 'Falha ao carregar picks de Monthly DY.',
    searchPlaceholder: 'Buscar por nome ou ticker…',
    yieldFilterLabel: 'Yield mínimo',
    yieldFilterAll: 'Todos os yields',
    sortLabel: 'Ordenar por',
    sortYieldDesc: 'Yield (maior → menor)',
    sortYieldAsc: 'Yield (menor → maior)',
    sortMarketCap: 'Cap. de mercado',
    sortRisk: 'Risco',
    yieldLabel: 'Yield',
    annualDividend: 'Anual',
    perPaymentLabel: 'Por pagamento',
    paymentsPerYear: '{n}× / ano',
    marketCap: 'Cap. mercado',
    price: 'Preço',
    sector: 'Setor',
    exchange: 'Bolsa',
    industry: 'Indústria',
    nextExDividendOn: 'Próx. ex-div {date}',
    nextExDividendIn: 'Próx. ex-div em {days} dias',
    nextExDividendToday: 'Vai ex-dividendo hoje',
    nextExDividendUnknown: 'Próx. ex-div não anunciada',
    nextPaymentOn: 'Pagamento {date}',
    lastDividendOn: 'Último pagamento {date}',
    pickCount: '{count} picks',
    riskTitle: 'Risco',
    riskLow: 'Baixo',
    riskMedium: 'Médio',
    riskHigh: 'Alto',
    riskLowDesc: 'REITs de patrimônio e ações de dividendos estabelecidos com fluxo de caixa estável.',
    riskMediumDesc: 'BDCs, infraestrutura de energia — modelos de negócio alavancados com sensibilidade ao fluxo de caixa.',
    riskHighDesc: 'REITs hipotecários, alavancados ou com distribuição volátil — cortes e erosão de capital são comuns.',
    assetEquityReit: 'REIT de patrimônio',
    assetMortgageReit: 'REIT hipotecário',
    assetBdc: 'BDC',
    assetEnergyInfra: 'Infraestrutura de energia',
    assetStock: 'Ação',
    assetEtf: 'ETF',
    backToList: 'Voltar para Monthly DY',
    priceHistoryTitle: 'Evolução do preço (5 anos)',
    priceHistorySince: 'Desde {date}',
    priceLabel: 'Preço',
    dividendMarkerLabel: 'Dividendo',
    dividendTooltipAmount: 'Dividendo: {amount}',
    distributionsTitle: 'Pagamentos (até os últimos 5 anos)',
    distributionsYearTotal: '{count} pagamentos · {total}',
    distributionsEmpty: 'Nenhum pagamento registrado.',
    keyFactsTitle: 'Dados principais',
    expenseRatio: 'Taxa de administração',
    notFound: 'Pick não encontrado na lista atual.',
  },
  follow: {
    listTitle: 'Empresas seguidas',
    listSubtitle: 'Tudo que você está acompanhando, em um só lugar.',
    loginRequired: 'Entre para seguir empresas e mantê-las aqui.',
    loading: 'Carregando sua lista...',
    emptyTitle: 'Nenhuma empresa seguida ainda',
    emptyBody: 'Siga empresas nas páginas de Picks IA ou Dividendos para vê-las aqui.',
    sourceAi: 'Pick IA',
    sourceDividend: 'Dividendo',
    sourceMonthlyDy: 'Monthly DY',
    sourceQuarterlyDy: 'Quarterly DY',
    filterAll: 'Todos',
    followedOn: 'Seguida em {date}',
    follow: 'Seguir',
    unfollow: 'Deixar de seguir',
    following: 'Seguindo',
    nextExDiv: 'Próx. ex-div',
    beatsInflation: 'Supera a inflação +{amount}%',
    losesInflation: 'Perde para inflação −{amount}%',
    matchesInflation: 'Acompanha a inflação',
    liveDataRefreshing: 'Atualizando dados…',
  },
  settings: {
    title: 'Configurações',
    subtitle: 'Olá {name} — ajuste como os picks são exibidos para você.',
    save: 'Salvar',
    saved: 'Salvo',
    clear: 'Limpar',
    error: 'Não foi possível salvar. Tente novamente.',
    taxCountry: {
      title: 'Residência fiscal',
      help: 'Usada para comparar o yield nominal de cada pick com a inflação do seu país, para você saber se está vencendo ou perdendo para a inflação.',
      label: 'País onde você declara impostos',
      placeholder: '— Selecione seu país fiscal —',
      tooltip: 'Escolha o país cuja inflação é relevante para você. Por enquanto suportamos apenas a UE-27 (os países para os quais temos dados de HICP).',
    },
    wht: {
      title: 'Imposto retido na fonte sobre dividendos',
      helpForUser: 'Quanto cada país de origem retém dos dividendos pagos a um residente fiscal de {country} — por padrão e após o envio da documentação prevista no tratado bilateral.',
      helpNoUser: 'Defina seu país fiscal acima para ver exatamente quanto cada país de origem retém dos dividendos pagos a você.',
      disclaimer: 'Apenas educacional — confirme com seu contador ou corretora antes de tomar decisões com base nesses números. As alíquotas de tratado pressupõem status de varejo e documentação correta; tratados bilaterais podem mudar.',
      searchLabel: 'Buscar país de origem',
      searchPlaceholder: 'Buscar país (ex. Alemanha, Suíça, EUA)…',
      statutory: 'Estatutária',
      treaty: 'Com tratado',
      colCountry: 'País de origem',
      colMechanism: 'Mecanismo',
      colNotes: 'Observações',
      appliedAtSource: 'Aplicada na fonte',
      requiresReclaim: 'Restituição necessária',
      noTreatyBenefit: 'Sem benefício de tratado',
      noResults: 'Nenhum país corresponde a “{query}”.',
      reviewedOn: 'Revisado em {date}. Sempre confirme com as fontes oficiais.',
      setCountryFirst: 'Defina seu país fiscal acima para ver as alíquotas específicas de retenção.',
    },
  },
}
